...
def attachment_list(request):
    if request.method == 'GET':
        items = Attachment.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def tag_list(request):
    if request.method == 'GET':
        items = Tag.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def bookmark_list(request):
    if request.method == 'GET':
        items = Bookmark.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def comment_list(request):
    if request.method == 'GET':
        items = Comment.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
        ...
        if serializer.is_valid():
            ...
            obj = Comment.objects.get(id=dict(serializer.data)['id'])
            publication = Publication.objects.get(id=request.data['publication'])
            publication.comments.add(obj)
            publication.save()
...
def publication_list(request):
    if request.method == 'GET':
        items = Publication.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
        serializer = PublicationSerializer(items, many=True)
        return Response(serializer.data)
...
def country_list(request):
    if request.method == 'GET':
        items = Country.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def state_list(request):
    if request.method == 'GET':
        items = State.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def city_list(request):
    if request.method == 'GET':
        items = City.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def neighborhood_list(request):
    if request.method == 'GET':
        items = Neighborhood.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def place_list(request):
    if request.method == 'GET':
        items = Place.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def achievementtype_list(request):
    if request.method == 'GET':
        items = AchievementType.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def achievement_list(request):
    if request.method == 'GET':
        items = Achievement.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def profile_list(request):
    if request.method == 'GET':
        items = Profile.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
...
def user_list(request):
    if request.method == 'GET':
        items = User.objects.order_by('pk').filter(**{key: dict(request.GET, **request.data)[key][0] for key in dict(request.GET, **request.data)})
